...
class YouTubeTTSv2:
    def __init__(self, root: tk.Tk):
        self.root = root
        self.root.title("YouTube Chat TTS — v2 (stability)")
    ...
    def load_voices(self) -> None:
        ...
        try:
            ...
            self.log(f"[WARN] โหลด voice list ไม่สำเร็จ: {exc}")
    ...
    def prune_recent_messages(self) -> None:
        if len(self.recent_messages) <= MAX_RECENT_MESSAGE_KEYS:
            return
        oldest = sorted(
            self.recent_messages.items(),
            key=lambda item: item[1][-1] if item[1] else datetime.min,
        )
        for key, _ in oldest[: len(self.recent_messages) - MAX_RECENT_MESSAGE_KEYS]:
    ...
    def chat_reader_process(url: str, ipc_queue) -> None:
        try:
            video_id = YouTubeTTSv2.extract_video_id(url)
    ...
    def tts_worker(self) -> None:
        ...
        while not self.tts_worker_stop.is_set():
            ...
            if not os.path.exists(filename):
                self.log("[ERROR] ไม่พบไฟล์เสียงที่สร้าง")
                continue
            ...
            while player.is_alive():
                ...
                if time.time() - start_time > MAX_PLAY_DURATION_SECONDS:
                    player.stop()
                    self.log("[WARN] playback timeout, stop forced")
                    break
                time.sleep(0.08)
    ...
    def cleanup_old_tts_files(self, max_keep: int = 10) -> None:
        ...
        for old_file in files[max_keep:]:
            try:
                ...
                self.log(f"[WARN] ลบไฟล์ไม่สำเร็จ: {old_file} ({exc})")
...
if __name__ == "__main__":
    ...
    root = tk.Tk()
    app = YouTubeTTSv2(root)
    app.log("[SYSTEM] พร้อมทำงาน ✅ (v2)")
